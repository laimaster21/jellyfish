...
ComputeThermalExpansionHeatSourceSmallStrainMieGruneisen::computeQpProperties()
{
  ...
  Real jacob, shock_heat,expansion_heat,vis_heat;
  ...
  if (jacob > 1.0) {// if expansion, penalize expansion heat at crack
    expansion_heat = expansion_heat * (1.0 - _c[_qp]) * (1.0 - _c[_qp]);
  }
  ...
  viscous_energy = _beta_v * viscous_energy * invce_ee_rate.trace()* (1.0 - _c[_qp]) * (1.0 - _c[_qp]);

...
InputParameters ACInterfaceCleavageFracture::validParams() {
...
ACInterfaceCleavageFracture::ACInterfaceCleavageFracture(
    const InputParameters &parameters)
    : ACInterface(parameters), _beta_penalty(getParam<Real>("beta_penalty")),
      _cleavage_plane_normal(
          getParam<RealVectorValue>("cleavage_plane_normal")) {}

Real ACInterfaceCleavageFracture::betaNablaPsi() {
  return _beta_penalty * _L[_qp] * _kappa[_qp] *
         (_grad_u[_qp] * _cleavage_plane_normal) *
         (_grad_test[_i][_qp] * _cleavage_plane_normal);
}

Real ACInterfaceCleavageFracture::computeQpResidual() {
  return (1 + _beta_penalty) * _grad_u[_qp] * kappaNablaLPsi() - betaNablaPsi();
}

Real ACInterfaceCleavageFracture::computeQpJacobian() {
  /// dsum is the derivative \f$ \frac\partial{\partial \eta} \left( \nabla
  /// (L\psi) \right) \f$
  RealGradient dsum = (_dkappadop[_qp] * _L[_qp] + _kappa[_qp] * _dLdop[_qp]) *
  ...
  if (_variable_L) {
    ...
    dsum += (_kappa[_qp] * dgradL + _dkappadop[_qp] * _phi[_j][_qp] * gradL()) *
            _test[_i][_qp];
  }

  return (1 + _beta_penalty) * _grad_phi[_j][_qp] * kappaNablaLPsi() +
         _grad_u[_qp] * dsum -
